...
module OpalScript
  class RakeTask
    ...
    def build_gem(name)
      ...
      puts " * #{out}"
      write_code OpalScript.build_gem(name), out
    ...
    def define_tasks
      ...
      task 'opal:build' do
        out = File.join @build_dir, "#{ @name }.js"
        puts " * #{out}"
        write_code OpalScript.build_files(@files, @dir), out
      end
      ...
      task 'opal:spec' do
        ...
        puts " * #{out}"
        write_code OpalScript.build_files(@specs_dir), out
      end
      ...
      task 'opal:dependencies' do
        ...
        puts " * #{out}"
        write_code OpalScript.runtime, out

        # build opal-parser?
        if @parser
          out = File.join @build_dir, 'opal-parser.js'
          puts " * #{out}"
          write_code OpalScript.parser_code, out
        end
      ...
      task 'opal:test' do
        runner = File.join OpalScript.core_dir, 'test_runner', 'runner.js'
